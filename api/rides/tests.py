...
class RideListAPITests(APITestCase):
    ...
    def test_list_rides(self):
        """Test that the API returns rides with the correct structure
        and filtering of ride events."""
    ...
    def test_filtering_by_status_and_rider_email(self):
        ...
        response = self.client.get(
            self.list_url, {"rider__email": self.non_admin_user.email}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        data = response.json()
        self.assertEqual(len(data["results"]), 2)
    ...
    def test_sorting_by_distance(self):
        """Test that rides are sorted by computed distance
        when latitude and longitude are provided."""
        self.client.force_authenticate(user=self.admin_user)
        response = self.client.get(
            self.list_url, {"ordering": "distance", "latitude": "0", "longitude": "0"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        rides = response.json()["results"]
    ...
    def test_minimal_query_count(self):
        """Ensure that the number of DB queries is minimized
        (expect 2 or 3 queries, including pagination count)."""
        self.client.force_authenticate(user=self.admin_user)
        with CaptureQueriesContext(connection) as queries:
            response = self.client.get(self.list_url)
            self.assertEqual(response.status_code, status.HTTP_200_OK)
            self.assertLessEqual(
                len(queries), 3, msg=f"Too many queries executed: {len(queries)}"
            )

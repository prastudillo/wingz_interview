...
class Command(BaseCommand):
    help = 'Clears existing rides and ride events and populates the Ride table with sample data and ride events'
    ...
    def handle(self, *args, **options):
        ...
        rider, _ = User.objects.get_or_create(
            username='rider1',
            defaults={'email': 'rider1@example.com', 'role': 'rider', 'password': 'pass'}
        )
        # Create or get a driver
        driver, _ = User.objects.get_or_create(
            username='driver1',
            defaults={'email': 'driver1@example.com', 'role': 'driver', 'password': 'pass'}
        )
        ...
        self.stdout.write(self.style.SUCCESS('Successfully created 20 rides with associated ride events.'))
